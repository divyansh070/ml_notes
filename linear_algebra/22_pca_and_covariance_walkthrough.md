...
* **Diagonal entries $\Sigma_{jj}$:** The sample variance $\operatorname{Var}(X_j)$ of feature $j$.
* **Off-diagonal entries $\Sigma_{jk}$:** The sample covariance $\operatorname{Cov}(X_j, X_k)$ between feature $j$ and feature $k$.
* **Symmetry:** Because $\operatorname{Cov}(X_j, X_k) = \operatorname{Cov}(X_k, X_j)$, $\Sigma$ is **always symmetric** ($\Sigma = \Sigma^T$).
...
$$
\max_{\mathbf{w}} \operatorname{Var}(X_c \mathbf{w}) = \max_{\mathbf{w}} \mathbf{w}^T \Sigma \mathbf{w} \quad \text{subject to} \quad \mathbf{w}^T \mathbf{w} = 1
$$
