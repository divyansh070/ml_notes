...
1. The projection $\mathbf{p} = \text{proj}_{\mathbf{a}}(\mathbf{b})$ lies on the line spanned by $\mathbf{a}$, so $\mathbf{p} = c \mathbf{a}$ for some scalar $c \in \mathbb{R}$.
...
$$
\mathbf{p} = \text{proj}_{\mathbf{a}}(\mathbf{b}) = \left( \frac{\mathbf{a}^T \mathbf{b}}{\mathbf{a}^T \mathbf{a}} \right) \mathbf{a}
$$
...
> The orthogonal projection $\mathbf{p} = \text{proj}_S(\mathbf{b})$ is the **unique point in subspace $S$ that minimizes the Euclidean distance to $\mathbf{b}$**:
>
> $$
> \text{proj}_S(\mathbf{b}) = \arg\min_{\mathbf{z} \in S} \|\mathbf{b} - \mathbf{z}\|_2^2
> $$
>
...
1. The projected vector $\mathbf{p}$ lies in $\text{Col}(A)$, so $\mathbf{p} = A\mathbf{x}$ for some weight vector $\mathbf{x} \in \mathbb{R}^n$.
...
If $P$ projects onto subspace $S = \text{Col}(A)$, then $(I - P)$ is the **orthogonal projection matrix onto the residual subspace $S^\perp = N(A^T)$**:
...
* Orthogonality verification: $\mathbf{e} \cdot \mathbf{a} = (0)(4) + (4)(0) = 0$ ✓.
...
$$
P^2 = \begin{bmatrix} 0.2 & 0.4 \\ 0.4 & 0.8 \end{bmatrix} \begin{bmatrix} 0.2 & 0.4 \\ 0.4 & 0.8 \end{bmatrix} = \begin{bmatrix} 0.04 + 0.16 & 0.08 + 0.32 \\ 0.08 + 0.32 & 0.16 + 0.64 \end{bmatrix} = \begin{bmatrix} 0.2 & 0.4 \\ 0.4 & 0.8 \end{bmatrix} = P
$$
