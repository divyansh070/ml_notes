...
- [ ] **1. Vector Dot Product & Norms:** Given $\mathbf{a} = [3, -4]^T$ and $\mathbf{b} = [2, 1]^T$, compute $\mathbf{a} \cdot \mathbf{b} = 2$, $\|\mathbf{a}\|_1 = 7$, $\|\mathbf{a}\|_2 = 5$, $\|\mathbf{a}\|_\infty = 4$, and $\cos\theta = \frac{2}{5\sqrt{5}}$.
...
- [ ] **4. Gaussian Elimination to Solve $A\mathbf{x} = \mathbf{b}$:** Solve the $2 \times 2$ system $2x_1 + x_2 = 5$ and $x_1 - x_2 = 1$ via forward elimination to row echelon form and back-substitution to get $\mathbf{x} = [2, 1]^T$.
...
- [ ] **6. Inversion by Adjugate / Cofactors ($3 \times 3$):** Given an upper triangular matrix $A = \begin{bmatrix} 1 & 2 & 0 \\\\ 0 & 3 & 1 \\\\ 0 & 0 & 2 \end{bmatrix}$, compute all 9 minors $M_{ij}$, cofactors $C_{ij} = (-1)^{i+j}M_{ij}$, construct the cofactor matrix $C$, transpose to get $\text{adj}(A) = C^T$, and divide by $\det(A) = 6$ to find $A^{-1}$.
- [ ] **7. Linear Independence Check:** Determine whether $\mathbf{v}_1 = [1, 2]^T$ and $\mathbf{v}_2 = [2, 4]^T$ are linearly dependent by checking if $\det([\mathbf{v}_1 \mid \mathbf{v}_2]) = 0$ or by finding non-trivial scalars $c_1, c_2$.
- [ ] **8. Rank via Row Echelon Form:** Reduce $A = \begin{bmatrix} 1 & 2 & 3 \\\\ 2 & 4 & 6 \\\\ 0 & 1 & 2 \end{bmatrix}$ to REF to identify the pivot count $\text{rank}(A) = 2$ and nullity $\dim(N(A)) = 3 - 2 = 1$.
- [ ] **9. Vector Projection:** Project $\mathbf{b} = [3, 4]^T$ onto $\mathbf{a} = [4, 0]^T$ using $\mathbf{p} = \frac{\mathbf{a}^T\mathbf{b}}{\mathbf{a}^T\mathbf{a}}\mathbf{a} = [3, 0]^T$, and verify orthogonality $\mathbf{a}^T(\mathbf{b} - \mathbf{p}) = 0$.
- [ ] **10. Eigenvalues & Eigenvectors ($2 \times 2$):** For $A = \begin{bmatrix} 2 & 1 \\\\ 1 & 2 \end{bmatrix}$, solve $\det(A - \lambda I) = \lambda^2 - 4\lambda + 3 = 0$ to get $\lambda_1 = 3, \lambda_2 = 1$. Solve $(A - \lambda I)\mathbf{v} = \mathbf{0}$ to find eigenvectors $\mathbf{v}_1 = [1, 1]^T, \mathbf{v}_2 = [1, -1]^T$.
- [ ] **11. Diagonalization & Matrix Power:** For $A$ above, diagonalize $A = P D P^{-1}$ and compute $A^3 = P D^3 P^{-1}$.
- [ ] **12. SVD Hand Calculation ($2 \times 2$ rank-1):** For $A = \begin{bmatrix} 1 & 1 \\\\ 1 & 1 \end{bmatrix}$, compute eigenvalues of $A^T A = \begin{bmatrix} 2 & 2 \\\\ 2 & 2 \end{bmatrix}$ ($\lambda_1 = 4, \lambda_2 = 0$), find singular value $\sigma_1 = \sqrt{4} = 2$, right singular vector $\mathbf{v}_1 = \frac{1}{\sqrt{2}}[1, 1]^T$, and left singular vector $\mathbf{u}_1 = \frac{1}{\sigma_1}A\mathbf{v}_1 = \frac{1}{\sqrt{2}}[1, 1]^T$. Verify $A = \sigma_1 \mathbf{u}_1 \mathbf{v}_1^T$.
...
- [ ] **29. Gram-Schmidt to QR Factorization:** Orthonormalize $\mathbf{a}_1 = [1, 1]^T, \mathbf{a}_2 = [1, 0]^T$ to find $\mathbf{q}_1, \mathbf{q}_2$, and construct upper-triangular matrix $R = Q^T A$.
