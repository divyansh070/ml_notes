...
Because observed targets contain noise and non-linearities, the vector $\mathbf{y}$ almost never lies within the Column Space $\operatorname{Col}(X)$. The system is **inconsistent**—no exact parameter vector $\mathbf{w}$ exists.
...
1. We want the predicted point $\hat{\mathbf{y}} = X\mathbf{w} \in \operatorname{Col}(X)$ that is closest to $\mathbf{y}$.
2. By the Closest Point Theorem, the shortest distance occurs when the residual error vector $\mathbf{e} = \mathbf{y} - X\mathbf{w}$ is **strictly perpendicular (orthogonal)** to the feature column space $\operatorname{Col}(X)$.
...
5. Rearranging gives the **Normal Equations**:
   ...
   $$
6. If $X$ has full column rank ($\operatorname{rank}(X) = n$), the $n \times n$ matrix $X^T X$ is invertible:
   $$
...
* Check sum of residuals: $1/3 - 2/3 + 1/3 = 0 \quad \checkmark$
* Check feature orthogonality: $X^T \mathbf{e} = \begin{bmatrix} 1/3 - 2/3 + 1/3 \\ 1(1/3) + 2(-2/3) + 3(1/3) \end{bmatrix} = \begin{bmatrix} 0 \\ 1/3 - 4/3 + 3/3 \end{bmatrix} = \begin{bmatrix} 0 \\ 0 \end{bmatrix} \quad \checkmark$
...
* $\operatorname{rank}(X) < n \implies \det(X^T X) = 0$. $X^T X$ cannot be inverted!
...
* **Inverting When $m < n$ (More Features Than Samples):** If you have 100 features and only 50 samples, $\operatorname{rank}(X) \le 50 < 100$. $X^T X$ is guaranteed to be singular. OLS will crash without regularization.
